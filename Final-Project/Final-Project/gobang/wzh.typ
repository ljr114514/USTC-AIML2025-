...
=== 11.1 学习收获
这门课的收获没有想象中的多，我初步认识了一些机器学习的方法，然后在ai的help（掌控）下简单的应用，算是简单的了解了ai model的训练过程，懂基本的超参数的作用，也学了一些常用的ai训练工具。
其实，我认为这门课还是很不错的，但我的收获并不多。下面，我简单分析一下失败原因。

=== 11.2 课程建议
老师一开始上课几乎全在讲一些数学原理，使我不知道这门课要干什么，我想大部分大二学生，既不了解机器学习的方法，又没有扎实的数学基础。所以对我来说，就是每周去上课听讲，然后不知道自己听了什么。而平时的HW又是对课堂内容的复现，拓展，那我做HW就只能完全依赖ai。然后恶性循环，上课不想听，HW用ai预习上课内容。

这里建议老师减少过于详细的数学推导，我感觉有些东西只需要明白能这样就可以了，可以多教一些关于算法的执行流程，优化策略等更实际，有用的东西。

然后是课程的LAB，虽然助教说会非常简单，但我是真的零基础，对于机器学习什么都不知道。刚写LAB1的时候，都不知道什么是完成了，所以根本就没有考虑模型优化。对于LAB2,3，依旧是没有一点自己的想法。

这里建议本课程多设计一些简单，内容少的小实验（就像LAB3的decision tree那样）在平时，这样在做大型的实验时至少会有一种熟悉的感觉。（一定要比较容易满分或分值比较低，不会有太大的心理负担）。
